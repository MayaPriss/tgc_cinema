...
function laksana_masa($tarikh,$id_filem)
{
  ...
  while($row=mysqli_fetch_array($laksanakan_arahan))
  {
    ...
    echo"<li><a class='w3-round-xxlarge w3-button w3-indigo' href='filem_seat.php?".http_build_query($data_get)."'>".$row['masa']."</a></li>";
}
  echo "</ul>";
}
...
<table class='w3-gray'>
  <tr valign='top'>
    ...
    <button class='w3-button w3-round-xlarge w3-black' type='submit'>CARI</button>
    </form>";

    # tab pilihan tarikh
    echo"
    <button class='tablink w3-black' onclick=\"opentarikh(event,'tarikh1')\"><b>".date('d M (D)',$hari_ini)."</b></button>
    <button class='tablink w3-black' onclick=\"opentarikh(event,'tarikh2')\"><b>".date("d M (D)", $esk)."</b></button>
    <button class='tablink w3-black' onclick=\"opentarikh(event,'tarikh3')\"><b>".date("d M (D)", $lusa)."</b></button> ";
